...
void Game::Render() {
  Texture2D myTex = ResourceManager::GetTexture("face");
...
void Game::DoCollisions(){
    ...
    if(!box.Destroyed){
      ...
      if(std::get<0>(collision)){
        ...
        if(dir == LEFT || dir == RIGHT){
          ...
          float penatration = Ball->Radius - std::abs(diff_vector.y);
          if(dir == UP)
            Ball->Position.y -= penatration;
          else
            Ball->Position.y += penatration;
        }
      }
  ...
  //We have checked all the bricks now time for the player
